...
const createMark = () => {
    ...
    liMessage.textContent = `${input.value}; Weight - `
    ...
    console.log(`Create Mark ${input.value}`);
...
const checkMarkFunction = () => {
    if(input.value === '') {
        error.textContent = 'Input number';
        error.style.color = '#bccad6';
    }
    else {
        error.textContent = ''
        if(input.value == 0) {
            error.textContent = 'Input another number than 0';
            error.style.color = '#bccad6';
        }
        else {
            if(weight.value == '' || weight.value == 0) {
                error.textContent = 'Input weight, another than 0';
                error.style.color = '#bccad6';
            }
            else {
                if(input.value.match(numbers)) {
                    ...
                    error.textContent = 'Input numbers 1-9';
                    error.style.color = '#bccad6';
                }
            }
        }
